...
## 2026-08-27 - SSRF via Unrestricted External Request in Admin Serializers
**Vulnerability:** In `AdminCategorySerializer` and `AdminProductSerializer` in `backend/apps/products/admin_serializers.py`, the `_download_image` method fetched user-supplied image URLs using `requests.get` without checking the destination hostname, enforcing HTTPS, or preventing redirects. This allowed potential SSRF attacks where an authenticated admin or attacker with access could provide an internal network URL (e.g., `http://169.254.169.254` or local service) and force the server to fetch it.
**Learning:** Downloading remote resources (like images) on the backend is inherently risky and requires strict validation. Without explicit checks, any URL provided in API payloads could be fetched, leading to internal network reconnaissance or interaction with local services.
**Prevention:** When downloading remote assets based on user input, strictly enforce the HTTPS scheme, check the hostname against a whitelist (e.g., using `IMAGE_URL_ALLOWED_HOSTS`), and pass `allow_redirects=False` to the HTTP client.
