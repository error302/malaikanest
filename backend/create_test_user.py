...
if not User.objects.filter(email=test_email).exists():
    ...
    if User.objects.filter(phone_number=candidate_phone).exclude(email=test_email).exists():
    ...
    user = User.objects.create_user(
        ...
        first_name="Test",
        last_name="User",
        is_email_verified=True,  # Mark as verified so they can login
        is_active=True
    )
    ...
    user.is_active = True
    if User.objects.filter(phone_number=test_phone).exclude(email=test_email).exists() and user.phone_number == test_phone:
        user.phone_number = "+2547" + str(abs(hash(test_email)) % 100000000).zfill(8)
    user.save()
