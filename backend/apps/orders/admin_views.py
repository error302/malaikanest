...
class AdminReportsView(APIView):
    ...
    def get(self, request):
        ...
        # Average order value
        average_order_value = total_orders > 0 else 0
        if total_orders > 0:
        ...
        orders_by_status = [{'status': s['status'], 'count': s['count']} for s in status_counts]
        ...
        revenue_by_month = [
            {'month': m['month'], 'revenue': float(m['revenue'] or 0)}
            for m in monthly
        ]
