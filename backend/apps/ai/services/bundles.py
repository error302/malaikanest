...
def generate_bundle(
    ...
    if bundle_type in BUNDLE_TEMPLATES:
        return _generate_template_bundle(
            bundle_type, budget, age_group, gender, climate
        )
...
Available categories: {", ".join(Category.objects.values_list("name", flat=True))}"""
    ...
    try:
        result = ollama_client.chat_json(
            message=prompt, system_prompt=BUNDLE_SYSTEM_PROMPT
        )
...
def _generate_template_bundle(
    ...
    for category_name, quantity in zip(template["categories"], template["quantities"]):
        category_products = Product.objects.filter(
            category__name__icontains=category_name.split()[0], is_active=True
        )[:quantity]
...
def _resolve_bundle_with_products(
    ai_response: Dict, budget: Optional[int]
) -> Dict[str, Any]:
    products = []
    total = 0
    ...
    for item in ai_response.get("items", []):
        ...
        category_products = Product.objects.filter(
            category__name__icontains=category_name, is_active=True
        )[:quantity]
...
def get_bundle_suggestions(
    budget: int, age_group: str = "newborn"
) -> List[Dict[str, Any]]:
    suggestions = []
