    ...
    def _download_image(self, image_url):
        ...
        try:
            ...
            if not (parsed.scheme == "https" and parsed.hostname and parsed.hostname.lower() in allowed_hosts):
                logger.warning("Rejected category image download from untrusted URL: %s", image_url)
                return None
...
class AdminProductSerializer(serializers.ModelSerializer):
    ...
    def _download_image(self, image_url, default_name):
        ...
        try:
            ...
            # SSRF guard: only fetch over HTTPS from an allow-listed host
            if not (parsed.scheme == "https" and parsed.hostname and parsed.hostname.lower() in allowed_hosts):
                logger.warning("Rejected product image download from untrusted URL: %s", image_url)
                return None
