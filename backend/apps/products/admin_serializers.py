...
class AdminCategorySerializer(serializers.ModelSerializer):
    ...
    def _download_image(self, image_url):
        ...
        try:
            ...
            allowed_hosts = getattr(settings, "IMAGE_URL_ALLOWED_HOSTS", ["res.cloudinary.com"])

            if parsed.scheme != "https" or not parsed.hostname or parsed.hostname.lower() not in allowed_hosts:
                logger.warning("Rejected category image download from unauthorized URL: %s", image_url)
                return None
...
class AdminProductSerializer(serializers.ModelSerializer):
    ...
    def _download_image(self, image_url, default_name):
        ...
        try:
            ...
            if parsed.scheme != "https" or not parsed.hostname or parsed.hostname.lower() not in allowed_hosts:
                logger.warning("Rejected product image download from unauthorized URL: %s", image_url)
                return None
