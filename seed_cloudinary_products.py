...
print(f"Found {len(image_files)} image files on host.")
...
for idx, item in enumerate(products_data):
    ...
    with open(img_path, 'rb') as f:
        product.image.save(clean_filename, File(f), save=True)

    status_str = 'Created' if created else 'Updated'
    print(f"[{status_str}] {product.name} -> Image saved via Cloudinary Storage: {product.image.url}")

print("All products successfully uploaded and saved with working Cloudinary URLs!")
