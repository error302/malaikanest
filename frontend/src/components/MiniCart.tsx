...
export default function MiniCart({ items = [], onRemove, onUpdateQty }: { items?: Item[]; onRemove?: (id: any) => void; onUpdateQty?: (id: any, qty: number) => void }) {
  ...
  return (
    <div className="w-80 bg-white rounded-xl shadow-xl p-4 border border-secondary">
      ...
      {items.length === 0 ? (
        <div className="text-sm text-gray-500 py-8 text-center">Your cart is empty</div>
      ) : (
      ...
      {items.length > 0 && (
        <div className="pt-3 mt-3 border-t border-secondary">
          ...
          <Link href="/checkout" className="block text-center w-full py-3 bg-cta hover:bg-cta-hover text-white font-semibold rounded-lg">Proceed to Checkout</Link>
          <Link href="/categories" className="block text-center w-full mt-2 py-2 text-sm text-gray-600 hover:text-accent">Continue Shopping</Link>
        </div>
      )}
    </div>
  )
