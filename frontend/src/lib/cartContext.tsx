...
function normalizeCartItem(item: ApiCartItem): CartItem {
  ...
  return {
    ...
    name: item.variant?.color_label
      ? `${item.product?.name || 'Product'} - ${item.variant.color_label}`
  ...
  const fetchCart = useCallback(async () => {
    try {
      const res = await api.get('/api/v1/orders/cart/', {
        headers: { 'Cache-Control': 'no-store', 'X-No-Auth-Redirect': 'true' },
      });
      dispatch({ type: 'HYDRATE', cartData: normalizeCartData(res.data) });
      setSynced(true);
    } catch (e) {
      // Silently fail — guest users without a session won't have a server cart
    }
  }, []);
    ...
    async (id: string | number, qty: number) => {
      ...
      const timer = setTimeout(async () => {
        ...
        try {
          ...
          dispatch({
            type: 'HYDRATE',
            cartData: { items: previousItems, subtotal: '0', total: '0' },
          });
          await fetchCart();
        } finally {
          setLoading(false);
